...
// ── Types re-declared locally (worker has no access to the parent module graph) ─
...
class StreamTransport {
  ...
  write(event: TrackerEvent): void {
    ...
    if (this.transport.rotation?.strategy === 'daily') {
      ...
      if (this.stream.bytesWritten >= this.bytesLimit) this.rotate()
    }
  ...
  private rotate(): void {
    ...
    try { fs.renameSync(this.currentPath, archived) } catch {}
  ...
  private cleanupOldFiles(): void {
    ...
    try {
      fs.readdirSync(dir)
        ...
        .slice(maxFiles)
        .forEach(({ name }) => { try { fs.unlinkSync(path.join(dir, name)) } catch {} })
    } catch {}
  }
...
parentPort?.on('message', (msg: { type: string; event?: TrackerEvent; transportIdx?: number }) => {
  if (msg.type === 'write' && msg.event) {
    if (LEVEL_NUM[msg.event.level] < minLevel) return

    // If transportIdx provided, write only to that transport; otherwise write all
    if (msg.transportIdx !== undefined) {
      transports[msg.transportIdx]?.write(msg.event)
    } else {
      for (const t of transports) {
        try { t.write(msg.event) } catch (err: unknown) {
