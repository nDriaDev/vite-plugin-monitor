import { AppTab, PRESETS, presetToRange, store, TimePreset, TimeRange } from "../state";
...
`;
	...
	presetBtns.forEach(btn => {
		on(btn, 'click', () => {
			const preset = btn.dataset.preset as TimePreset | 'custom';
			if (preset === 'custom') {
				// INFO Show inputs pre-filled with current range
